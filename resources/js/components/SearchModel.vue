	...
	<div class="">
		...
		<div class="">
			<div v-if="searchStatus" class="lg:hidden md:hidden inline-flex absolute top-8 left-0 w-full mt-0 z-20 pt-2 bg-gray-400 w-full p-2">
				<input  @keyup.prevent="searchUser" type="text" class="relative w-full text-gray-700 rounded-full px-4 py-1 pr-10" v-model="searchKey">
				<svg @click.prevent="enlargeSearch" class="absolute right-0 z-10 mr-1 h-8 w-8 text-gray-700 pr-2" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><polygon points="16.172 9 10.101 2.929 11.515 1.515 20 10 19.293 10.707 11.515 18.485 10.101 17.071 16.172 11 0 11 0 9"/></svg>
			</div>
		...
		<div class="relative">
			<div class="absolute right-0  mt-4 m-2 w-48 bg-gray-400 rounded"  >
				<div class="flex items-end w-full mb-2" v-for="u in users">
					<a :href="`/profile/${u.id}`" class="mr-2">
						<img v-if="u.profile.avatar" class="user-img-sm" :src="'/storage/'+ u.profile.avatar">
